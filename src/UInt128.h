...
{
	public:
		void directAssign(ULONG value0,ULONG value1,ULONG value2,ULONG value3);
			CUInt128();
			CUInt128(bool bFill);
			...
			void setPrefixBits(unsigned short prefixBase,unsigned int prefixLen,unsigned int valueToSet);
			...
			void ToHexString(string *pstr) const;
			string ToHexString() const;
			void FromHexString(string hexString);
			void ToBinaryString(string *pstr, bool bTrim = false) const;
			void ToByteArray(byte *pby) const;
			ULONG Get32BitChunk(int iVal) const;
			unsigned char GetByteChunk (unsigned int pos) const;
